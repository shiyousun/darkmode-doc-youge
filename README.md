...
- 🌟 **温柔反色（gentle_invert）**：图片不是简单 invert，而是反色后用 LUT 把白色压到主题暖色，跟文档融为一体，整页色调统一不刺眼
...
python3 scripts/darkmode_batch.py "/path/to/your/dir" --theme gray
python3 scripts/darkmode_batch.py "/path/to/your/dir" --theme pure
```
...
# DOCX
python3 scripts/darkmode_docx.py input.docx [output.docx] [--no-images] [--theme warm]
...
> 实测处理 24 个中考试卷文档（12 PDF + 11 DOCX + 1 XLSX），覆盖 6 个学科子目录，全部成功，耗时 134 秒。
...
| `scripts/darkmode_docx.py` | DOCX 反色（背景 + 文字 + 表格 + 内嵌图片三处统一处理） |
| `scripts/darkmode_xlsx.py` | XLSX 反色（单元格 + 字体 + 边框） |
| `scripts/darkmode_image.py` | 单图反色（含 EXIF 旋转校正） |
| `scripts/darkmode_batch.py` | 批处理入口（递归扫描 + 异常隔离 + 汇总报告） |
...
- `Pillow` — 图像反色
- `openpyxl` — XLSX 处理
- `LibreOffice` (`soffice`) — 仅在转 .doc 时需要（可选）
